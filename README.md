...
종류
정수 : byte, short, int(기본형), long(초기화 시 뒤에 L을 붙인다)
실수 : float(초기화 때 뒤에 f를 붙인다), double(기본형)
문자 : char
논리형 : boolean
참조형(Reference Type)
...
특수문자는 '_'와 '$'만을 허용한다.
카멜 표기법을 사용한다.
한글입력이 가능하지만 사용을 지양한다.
의미있는 변수명을 사용하는 것을 권고한다.
상수
정의
...
정수형 : sc.nextByte(), sc.nextShort(), sc.nextInt(), sc.nextLong()
실수형 : sc.nextFloat(), sc.nextDouble()
문자열 : sc.next(), sc.nextLine()
문자 : sc.next().charAt(0), sc.nextLine().charAt(0)
제작 프로그램
...
증감 연산자
전위 연산과 후위 연산의 차이
Java에서 연산은 앞에 2개만 먼저 계산을 하기에 앞쪽에 증감 연산자가 후위에 있더라도 앞에 2개가 계산이 끝나면 증감 연산자가 계산이 됨.
ex) a = 10, b = 10, c = (a++) + (++b) + a이면 c는 32
비트반전
각 비트에서 0을 1로, 1을 0으로 반전한다.
ex) 1011 -> 0100
3순위 : 산술 연산자(*, /, %)
4순위 : 산술 연산자(+, -)
5순위 : 시프트(>>, <<, >>>)
...
선언과 동시에 값을 기록할 수 있음
int[] arr = {1, 2, 3, 4, 5}; : 길이가 5인 정수형 배열을 생성하면서 각 인덱스에 1, 2, 3, 4, 5 순서대로 저장
String[] arr = {"hi", "hello", "bye"}; : 길이가 3인 문자열 배열을 생성하면서 각 인덱스에 "hi", "hello", "bye"를 순서대로 저장
...
참조자료형 : 반환 값이 참조형 자료형일 경우(String, 사용자 정의)
배열 : 기본형 배열, 참조형 배열 모두 가능
메소드의 매개변수
() : 매개변수가 없는 것을 의미함
기본자료형 : 기본형 매개변수 사용 시 값을 복사하여 전달. 메소드에서 해당 값을 변경해도 원래 값은 변경 X => call by value
참조자료형, 배열 : 참조자료형, 배열을 매개변수로 전달 시에는 데이터의 주소값이 전달. 메소드에서 해당 값을 변경하면 원래 값도 같이 변경 => call by reference
메소드의 오버로딩
매개변수의 갯수가 다른 메소드는 동일한 이름으로 선언 가능.
서로 다른 타입의 매개변수로 가진 메소드는 동일한 이름으로 선언 가능.
매개변수가 동일한 타입이라면 리턴 타입이 다르더라도 동일한 이름으로 선언 불가능.
객체지향 프로그래밍
...
데이터를 운영하는 클래스 ex) Control Class
데이터의 CRUD
Create
Read
Update
...
접근제어 지시자는 수정이 가능하지만 부모의 것보다 같거나 넓은 범위로 가능하다.
제작 프로그램
...
추상 클래스는 객체의 생성이 불가능.
추상 클래스의 특징
미완성 클래스(abstract 키워드 사용)로 자체적으로 객체 생성 불가 -> 반드시 상속하여 객체를 생성
abstract 메소드가 포함된 클래스 -> 반드시 abstract 클래스. 단, abstract 메소드가 없어도 abstract 클래스 선언이 가능(객체 생성 방지용)
abstract 메소드 외에 일반 메소드와 변수를 포함할 수 있음
객체 생성은 안되나 참조형 변수 type으로는 사용 가능(다형성으로는 가능)
인터페이스
...
Wrapper.parseInt(str), parseDouble(str) 등 : 문자열을 각 타입의 기본 자료형으로 변환, char 제외(charAt 메소드 사용)
Wrapper.valueOf(num).toString : 각 기본 자료형을 문자열로 변환
String.valueOf(num) : String 클래스의 valueOf 메소드로 사용하여 기본 자료형을 문자열로 변환
제네릭(Generics)
...
주로 개발자의 부주의로 생기는 오류
Exception의 예
ArithmeticException : 숫자를 0으로 나눌 때 생기는 오류
ArrayIndexOutOfBoundsException : 배열 크기 오류(5개의 크기로 선언된 배열 num이 num[-1]이나 num[5]와 같이 범위를 넘어선 경우)
NullPointerException : Null인 레퍼런스의 변수나 메소드 참조 시도시 발생
NegativeArraySizeException : 배열의 크기를 음수 또는 0으로 지정하는 경우 발생
ClasscastException : cast 연산자 사용시 타입 오류일 때 발생(up-casting, down-casting)
InputMismatchException : 정수를 입력해야 하는데 문자가 입력된 경우
예외의 처리
try~catch문을 사용하여 발생한 곳에서 직접 처리
throws를 사용하여 예외를 던질 수 있음
...
클라이언트의 접속 요청을 기다림 3. 요청이 오면 수락 4. 클라이언트 정보를 저장
- Socket client = server.accept();
클라이언트 정보를 통해서 OutputStream 생성
- DataOutputStream dos = new DataOutputStream(client.getOutputStream());
클라이언트 정보를 통해서 InputStream 생성
- DataInputStream dis = new DataInputStream(client.getInputStream());
모든 데이터를 주고 받으면 InputStream과 OutputStream, 그리고 클라이언트와 연결된 Scoket을 close()시켜준다.
Socket
...
JAVA SE(Standard Edition) : 자바 표준 라이브러리가 제공되는 버전. 일반적으로 가장 많이 사용된다.
JAVA EE(Enterprise Edition) : 자바를 이용해서 서버 개발을 위한 플랫폼으로, 표준 라이브러리 외 서버 동작을 위한 다양한 라이브러리가 추가 제공된다.
JAVA ME(Micro Editon) : 휴대전화, 셋톰박스와 같이 작은 규모에서 자바를 지원하기 위한 플랫폼
자바 설치 -> 환경 변수 설정
웹 서버 구축
종류
서버 : 서비스를 제공하는 컴퓨터.
클라인트 : 서비스를 이용하는 컴퓨터.
클라이언트가 서버를 찾아가는 방법
IP 주소 : 네트워크 상에서 통신 기기들이 통신을 위해 사용하는 고유주소(IPv4, IPv6)
포트 번호 : 네트워크 상에서 특정 서버 프로그램을 지정
프로토콜 : 컴퓨터와 컴퓨터가 정보를 주고 받을 때 통신 방법에 대한 규약
웹 서비스 프로토콜 : http://
TCP : 연결지향형 통신으로 데이터 전달의 신뢰성을 최대한 보장하는 프로토콜
UDP : 비연결지향형 통신으로 빠른 데이터 전달을 보장하는 프로토콜
DB
Data : 어떠한 자료 또는 실제 값
...
실시간 서비스
데이터의 값이 시간에 따라 변화
서로 다른 업무 또는 여러 사용자에게 동시에 공유가 된다.
저장된 값에 따라 참조가 된다.
DBMS의 특징
데이터간의 관계를 정리하고 처리를 쉽게 해준다.
하나의 DB가 여러 사용자가 사용할 수 있도록 다양하고 쉬운 인터페이스를 제공
중복 데이터를 최소화하고, 유지 보수가 편하다.
데이터 복제본을 저장하는 기능이 존재
여러 사람이 사용할 때 데이터간 불일치 방지
접근 제어 가능
Front end
사용 언어
...
Table 용어
Relation : 테이블의 이름을 Relation이라고 부름.
Column(Attribute, Domain) : 특정한 단순 자료형과 데이터의 열
Row : 테이블에서 한 객체의 대한 전체 정보를 표현한 행(가로)
...
문자
CHAR(SIZE) : 고정 길이 문자(최대 2000byte) - 데이터의 공백을 빈 칸으로 채워 넣는다. but, 데이터 처리 속도가 빠르다.
SIZE : 포함될 문자(열) 크기
...
VARCHAR2 : 가변 길이 문자(최대 4000byte) - 데이터의 공백은 버린다.
...
날짜의 연산 및 비교가 가능
날짜 + 숫자 : DATE - 더한 숫자만큼 며칠 후
날짜 - 숫자 : DATE - 더한 숫자만큼 며칠 전
날짜 - 날짜 : NUMBER - 두 날짜의 차이(일 수)
날짜 + 날짜 / 24 : DATE - 날짜 + 시간
데이터
...
=<, >= : 크거나 같다, 작거나 같다
<>, != ^= : 다르다
BETWEEN AND : 비교하려는 값이 지정한 범위(경계 포함 -> 이상/이하)에 포함되면 TRUE 리턴
LIKE : 비교하려는 값이 특정 패턴을 만족시키면 TRUE를 리턴하는 연산자 '%'와 '_'를 와일드카드로 사용할 수 있다.
%는 글자 길이에 제한이 없고(다른 글자와 동일한 들어가면 무조건 TRUE), 는 제한이 있다.(''의 칸 수만큼만 임의의 글자가 들어갈 수 있다.)
입력받은 _ 혹은 %가 실제 문자인지 와일드카드인지 알아보기 위해 ESCAPE 기능을 사용한다.
SELECT EMP_NAME, EMAIL FROM EMPLOYEE
WHERE EMAIL LIKE'___#_%' ESCAPE '#';
ESCAPE 문자는 다른 것이 들어와도 상관 없다.
NOT LIKE : 해당 문자가 들어가지 않는 경우에만 TRUE를 리턴한다.
IS NULL/IS NOT NULL : NULL 여부를 비교하는 연산자
IN, NOT IN : 비교하려는 값 목록에 일치하는 값이 있으면 TRUE를 반환하는 연산자
...
함수
함수의 유형
단일 행 함수 : 각 행마다 반복적으로 적용. 입력받은 행의 개수만큼 결과 반환
그룹 함수 : 특정한 행들의 집합으로 그룹을 형성. 그룹당 1개의 결과를 반환
문자 처리 함수
CHARACTER -> NUMBER
LENGTH : 문자열의 길이를 반환
...
LTRIM/RTRIM : 주어진 문자열의 왼쪽(LTRIM), 오른쪽(RTRIM)부터 시작해서 해당 문자열을 자르고 문자열을 반환한다.
LTRIM(STRING, STR)
RTRIM(STRING, STR)

- LTRIM은 왼쪽에서부터, RTRIM은 오른쪽에서부터 시작해 CHAR을 자르을 자른다.
- 만약 STR이 여러개의 문자면 해당하는 문자 모두를 제거한다.
TRIM : 주어진 컬럼 문자열의 앞/뒤/양쪽에 있는 지정한 문자를 제거한 나머지를 반환
...
LOWER/UPPER/INITCAP : 해당 문장을 대문자, 소문자, 첫글자만 대문자 변환
LOWER(STRING)
UPPER(STRING)
INITCAP(STRING)
...
CONCAT : 컬럼의 문자 혹은 문자열을 두 개 전달받아 하나로 합친 후 리턴
CONCAT(STRING, STRING)

- 입력받은 2개의 문자열을 합쳐 리턴한다.
REPLACE : 전달받은 문자열 중 지정한 문자를 전달받은 문자로 변환하여 리턴
...
ABS : 절대값을 구하여 리턴
ABS(NUMBER)
MOD : 입력 받은 수를 나눈 나머지 값을 리턴
MOD(NUMBER, DIVISION)

- 나눠지는 수(NUMBER)와 나누는 수(DIVISION)를 입력한다.
ROUND : 인자로 전달받은 숫자 혹은 컬럼의 소수점 반올림을 한다.
ROUND(NUMBER, [POSITION])

- 소수점 반올림을 할 위치를 입력한다. 입력받은 위치만큼 출력한다.
- 입력을 안할경우 정수로 반올림한다.
- POSITON이 음수가 되면 소수점 위로 올라간다.
FLOOR : 인자로 전달받은 숫자 혹은 컬럼의 소수점 아래를 버린다.
FLOOR(NUMBER)
CEIL : 인자로 전달받은 숫자 혹은 컬럼의 소수점 아래를 올린다.
CEIL(NUMBER]
TRUNC : 인자로 전달받은 숫자 혹은 컬럼의 소수점 아래를 버린다.
...
TO_CHAR : 날짜형 데이터 또는 숫자형 데이터를 문자형 데이터로 변환하여 리턴
TO_CHAR(DATE, [FORMAT])
...
TO_DATE : 숫자형 데이터 또는 문자형 데이터를 날짜형 데이터로 변환하여 리턴
...
선택함수
단일값
DECODE : 여러가지 경우에 선택할 수 있는 기능을 제공(일치하는 값)
DECODE(조건식, 조건1, 결과1, 조건2, 결과2)
범위값
CASE : 여러가지 경우에 선택할 수 있는 기능을 제공(범위 값 가능)
...
서브쿼리와 비교할 항목은 서브쿼리의 SELECT한 항목의 개수와 자료형이 일치해야함
서브쿼리 유형
단일행 서브쿼리
...
다중행과 다르게 일부 등호 계열 비교 연산자(=, !=, <> 등)는 가능하다. ex)
...
RESOURCE : CREATE 구문을 통해 객체를 생성할 수 있는 권한과 INSERT, UPDATE, DELETE 구문을 사용할 수 있는 권한을 모아둔 ROLE
사용법
...
PRIMARY KEY	NULL을 허용하지 않고, 중복을 허용하지 않는다.
컬럼의 고유 식별자로 사용하기 위함.
...
ON DELETE RESTRICTED : 아무것도 지정하지 않는 경우 설정되는 기본 삭제 옵션으로 자식 테이블에서 부모 테이블의 데이터를 참조하고 있는 경우 데이터의 삭제가 불가능하다.
ON DELETE SET NULL : 부모 테이블의 데이터 삭제 시 해당 데이터를 참조하고 있던 자식 테이블의 컬럼 값을 NULL로 변경하는 옵션
ON DELETE CASCADE : 부모 테이블의 데이터 삭제 시 해당 테이터를 참조하고 있던 자식 테이블의 데이터까지 모두 삭제하는 옵션
사용법
- 테이블 레벨에서 설정
...
TCL 종류
구문	설명
COMMIT	트랜잭션 작업이 정상 완료되면 변경내용을 저장
-> 모든 SAVEPOINT 삭제
ROLLBACK	트랜잭션 작업을 모두 취소하고 최근 COMMIT 시점으로 이동
SAVEPOINT	현재 트랜잭션 작업 시점의 이름을 지정
...
뷰에 포함되지 않은 컬럼 중에 베이스가 되는 테이블 컬럼이 NOT NULL 제약조건이 지정된 경우 INSERT가 불가능
산술 표현식으로 정의된 경우
JOIN을 이용해 여러 테이블을 연결한 경우
DISTINCT를 포함한 경우
그룹함수나 GROUP BY 절을 포함한 경우
옵션
CREATE OR REPLACE : 생성한 뷰가 없으면 새로 생성하고, 이미 존재하면 갱신
CREATE [OR REPLACE] 뷰이름
AS
SELECT문;
FROCE/NOFOCE : FORCE 옵션은 기본 테이블이 존재하지 않더라도 뷰를 생성. 기본값은 NOFORCE로 지정.
CREATE 뷰이름
AS
...
옵션
START WITH 숫자 : 처음 발생시킬 시작값(기본 1)
INCREMENT BY 숫자 : 다음 값에 대한 증가치(기본 1)
MAXVALUE 숫자/NOMAXVALUE : 최대값 지정
MINVALUE 숫자/NOMINVALUE : 최소값 지정
CYCLE/NOCYCLE : 시퀀스 최대값 도달 시 CYCLE은 START WITH 값으로 되돌아가고 NOCYCLE은 에러
CAHCE/NOCACHE : 메모리상에서 시퀀스값 관리(기본 20)
SEQUENCE 사용법
시퀀스명.CURRAVL
...
/
PLSQL 변수의 종류
일반(스칼라) 변수 : 기존 SQL 자료형과 유사값을 대입(:=)하고 변경하여 사용이 가능
상수 : 일반변수와 유사하나 CONSTANT 키워드가 자료형 앞에 붙고 선언시 값을 할당해주어야 하며 변경불가
DECLARE
  변수 CONSTANT 타입 := 값;
...
END;
...
- 초기화된 이후에는 변경 불가
%TYPE : 이전에 선언 된 다른 변수 또는 테이블의 컬럼 자료형에 맞추어
DECLARE
  변수 테이블명.컬럼명%TYPE
...
END;
/
%ROWTYPE : %TYPE과 유사하게 참조할 테이블의 컬럼데이터 타입을 자동으로 가져오나 1개의 컬럼이 아니라 여러개의 컬럼 값을 자동으로 가져옴
DECLARE
  변수 테이블명%ROWTYPE
...
END;
...
RECORD : %ROWTYPE이 참조할 테이블의 컬럼 데이터 타입을 자동으로 가져오는 반면 레코드는 직접적으로 컬럼타입 지정
DECRALE
...
END;
/
IF ~ THEN ~ ELSE ~ END IF문(JAVA의 if~else문)
DECLARE
  ...
BEGIN
...
END;
/
IF ~ THEN ~ ELSIF ~ ELSE ~ END IF문(JAVA의 if~else if문)
DECLARE
  ...
BEGIN
...
종류
DBA_XXXXX : 데이터베이스 관리자만 접근이 가능한 객체 등의 정보를 조회
ALL_XXXXX : 자신의 계정이 소유하거나 권한을 부여받은 객체 등에 관한 정보 조회
USER_XXXXX : 자신의 계정이 소유한 객체 등에 관한 정보 조회
...
ojdbc 사용하기 위한 사전 작업
C:\oraclexe\app\oracle\product\11.2.0\server\jdbc\lib 폴더 밑에 ojdbc6 파일 복사
C:\Program Files\Java\jdk1.8.0_144\jre\lib\ext 폴더 밑에 붙여넣기
이클립스 셋팅 : 문자 set 인코딩
...
General -> Editors -> Text Editors -> Spelling -> Encoding -> Other -> UTF-8
...
PreparedStatement
캐시를 사용한다.
select * from member where member_id = ?를 먼저 검사를 한다.
user01을 검색한다.
검사를 하지 않고 user02를 바로 검색한다. => 캐시를 하기에 검사를 안하고 바로 검색하기에 빠르게 검색이 가능하다.
보안성이 뛰어나다
...
Web Version
Web 1.0
정적인 검색 서비스 제공(초기 서비스 버전)
동적인 검색 서비스 제공(현재 가장 많이 사용되는 버전) -> 동적인 검색 및 사용자의 참여, 공유, 개방
Web 2.0
지능형 웹서비스
사용자의 패턴을 분석하여 개인별 맞춤 정보 검색 서비스 제공
youtube, facebook, Instagram 등등
Web 처리 과정
Client가 브라우저에 www.google.com을 입력
www.google.com의 IP 주소를 DNS에 질의
DNS는 www.google.com을 IP로 변경
...
구조 파악이 어려워 유지보수가 어렵고, 검색 엔진이 확인할 때에도 구별이 어려움 20200324010805
<div id = 'header'>
  헤더 내용
</div>
<div id = 'content'>
...
footer에는 header, section, article 등 다른 레이아웃 사용 가능 20200324010832
<header>
  헤더 내용
</header>
<section>
...
img 확장자
이미지 확장자	설명
GIF	파일 크기가 작아 작은 아이콘이나 블릿 기호에 많이 사용
...
속성	설명
method	get : URL 창에 데이터를 보내는 방식. 보내는 데이터 크기에 제한이 있음(256~4096byte) 보내는 데이터가 눈으로 보임.
post : http 헤더에 데이터를 넣어 보내는 방식. 보내는 데이터의 크기에 제한이 없음. 보내는 데이터를 볼 수 없음.
name	form 태그의 고유 이름 지정
action	데이터를 처리할 프로그램(페이지) 지정
target	action 속성을 어떻게 열것인지 결정
autocomplete	이전 입력내용 출력하는 기능(생략하면 자동으로 on)
사용법
...
value	input 요소의 기본값 입력(사용자 입력 값이 value가 됨)
name	input을 구별 할 수 있는 명칭(데이터를 받는 곳에서 name속성의 값으로 구분)
...
password	비밀번호 입력창, 입력 시 •••로 표시됨
hidden	사용자에게 보이지는 않지만 값을 넣을 수 있는 창, 데이터를 숨겨서 전송할 때 사용
button	버튼 생성, 자체기능은 없고, script를 통해 함수 연결 가능
checkbox	체크박스생성(다중 항목 선택)
radio	라디오버튼생성(단일 항목 선택)
file	파일입력 양식
submit	입력한 데이터를 태그의 action의 페이지로 전송
reset	입력한 내용을 지우는 기능
image	이미지 형태 생성
name	input의 구분자/명칭
size	사용자 화면에 보여줄 문자 수
value	input요소에 들어가 값(설정하지 않으면 빈값)
maxlength	최대 입력 갯수
minlength	최소 입력 갯수
...
선택자:nht-child(수열) / 선택자:nth-last-child(수열)
...
CSS 단위 구성
구분	단위	내용
상대 크기	em	부모 요소의 크기가 기준
배수를 의미
1배 = 1em = 100%(부모요소 크기 기준)
rem	최상이 부모 요소의 크기가 기준으로 배수
...
영문색 이름	영문으로 색 이름 작성
red, blue, white...
16진수	rgb값을 기준으로 16진수로 작성
#ff0000(빨간색), #000000(흰색), #0000ff(파란색)...
rgb	rgb값을 0~255의 숫자로 표현
rgb(255, 0, 0), rgb(0, 0, 0), rgb(0, 0, 255)...
rgba	rgb 방식과 동일하며 맨 마지막에 투명도를 0~1로 표현
rgba(255, 0, 0, 0.5)
0~1로 갈수록 투명->불투명
텍스트 스타일
front-family
...
bolder	더 굵게 표시
lighter	더 가늘게 표시
100~900	400:normal / 700:bold / 세밀한 조절 가능
font-variant
...
normal	하나로 처리	개행하지 않음	자동 줄 바꿈
nowrap	하나로 처리	개행하지 않음	한줄로 표시
pre	여러 개로 처리	개행 처리 함	한줄로 바꿈
pre-line	하나로 처리	개행 처리 함	자동 줄 바꿈
pre-wrap	여러 개로 처리	개행 처리 함	자동 줄 바꿈
...
lower-roman/upper-roman	로마자 소문자 / 로마자 대문자
...
inline	인라인 요소로 변경
block	블록 요소로 변경
inline-block	블록 요소와 같이 너비, 높이 지정 등은 가능하나, 배치는 인라인 요소처럼 나란히 배치되는 속성
테두리 스타일
border-style
...
outset	테두리를 포함한 컨텐츠 영역이 밖으로 튀어나온 것 처럼 보임
...
속성 값	내용
content-box	default값으로 컨텐츠까지 영역으로 설정
...
속성 값	내용
visible	요소가 화면에 보임(default)
hidden	요소가 화면에서 안보임<단, 보이지만 않을 뿐 공간은 차지
z-index
...
scale(x,y)	지정한 크기만큼 x,y축으로 확대
scale(x)	지정한 크기만큼 x축으로 확대
scale(y)	지정한 크기만큼 y축으로 확대
rotate(각도)	지정한 각도만큼 회전
트랜지션
웹 요소의 스타일이 바뀌는 것
애니메이션과 같은 효과를 나타낼 수 있음
속성 값	내용
transition-property	적용 할 속성 선택
...
animation-direction	종료 후 처음부터 시작 또는 반대로 시작 선택
...
확인 버튼 선택 시 true, 취소 버튼 선택 시 false 리턴
...
변수 선언
구분	설명	변수 선언
멤버변수(전역변수)	페이지 내 여러 함수에서 사용가능한 변수	변수명 = 값;
window.alert(내용);	내용을 메세지창에 출력(window는 생략 가능)	
- javascript에서는 자료형이 여러 개 존재 하지만 선언 할 때에는 var 로만 선언 - 대입하는 데이터에 맞춰서 자료형이 생성
자료형
문자열
"", ''로 묶여있는 리터럴
...
메소드	설명	비고
escape()	적절한 정도로 인코딩	알파벳, 숫자, @, *, -, /, .를 제외한 모든 문자
unescape()	escape()로 인코딩 된 값을 디코딩
encodeURI()	최소한의 문자만 인코딩	인터넷 주소에 사용되는(:,;,/,=,?,&) 변환 제외
decodeURI()	encodeURI()로 인코딩 된 값을 디코딩
encodeURIComponent()	대부분의 문자 인코딩	알파벳과 숫자를 제외한 모든 문자 인코딩
decodeURIComponent()	encodeURIComponent()로 인코딩 된 값을 디코딩
내장함수
메소드	설명
...
- 절차 1. 요소 노드 생성 2. 텍스트 노드 생성 3. 요소 노드에 텍스트 노드 추가 4. body 내부의 필요한 위치에 요소 노드 추가
텍스트 노드가 없는 문서객체 생성
...
객체명.appendChild(node)	태그에 자손으로 노드 추가
- 절차 1. 요소 노드 생성 2. 생성된 노드 속성 설정 3. body 내부의 필요한 위치에 요소 노드 추가
문서 객체 스타일 수정
...
$('선택자').filter('선택자')	일치하는 요소만 리턴
$('선택자').not('선택자')	인자값과 일치하지 않는 요소만 리턴
Ancestors 메소드
선택된 요소의 상위 요소들을 선택할 수 있는 메소드
메소드	설명
$('선택자').parent()	선택된 요소의 바로 위 상위요소만 리턴
$('선택자').parents([인자])	선택된 요소의 모든 상위요소 리턴
단, 매개변수가 있는 경우 매개변수와 일치하는 부모만 리턴
$('선택자').parentsUntil([인자])	선택된 요소부터 인자 요소까지 범위의 요소 리턴
단, 인자로 선택된 요소는 제외
Descendants 메소드
선택된 요소의 하위 요소들을 선택할 수 있는 메소드
메소드	설명
$('선택자').children([인자])	선택된 요소의 모든 자손(다음레벨) 객체를 리턴
인자를 입력하면 선택된 요소의 인자와 일치하는 자손객체 리턴
$('선택자').find([인자])	선택된 요소의 인자와 일치하는 모든 후손(모든 레벨) 객체 리턴
Sideways 메소드
선택된 요소의 같은 레벨에 있는 요소(형제)를 선택할 수 있는 메소드
메소드	설명
$('선택자').siblings([인자])	선택된 요소의 같은 레벨에 있는 요소 리턴
단, 매개변수가 있는 경우 매개변수가 있는 형제 요소 리턴
$('선택자').next()	선택된 요소의 같은 레벨 중 바로 다음 한 개의 요소 리턴
$('선택자').nextAll()	선택된 요소의 같은 레벨 중 선택된 요소의 다음 모든 요소 리턴
$('선택자').nextUntil('인자')	선택된 요소부터 같은 레벨들 중 인자까지 범위의 모든 다음 요소 리턴
$('선택자').prev()	선택된 요소의 같은 레벨 중 선택된 요소 바로 이전 한 개 요소 리턴
$('선택자').prevAll()	선택된 요소부터 같은 레벨 중 선택된 요소 이전의 모든 요소 리턴
$('선택자').prevUntil('인자')	선택된 요소부터 같은 레벨들 중 인자까지 범위의 모든 이전 요소 리턴
요소가 있는지 찾는 메소드
선택자로 지정된 범위에 특정한 요소가 존재하는지 찾을 때 사용하는 메소드
메소드	설명
$('선택자').is('인자')	선택된 요소의 범위내에 인자와 동일한 요소가 있는지 찾아서 있으면 true, 없으면 false리턴
...
구분	내용
HTML 태그 생성	직접 html 구문을 문자열로 작성
속성 설정	$(생성태그).css() | html() | attr() 등
- HTML 태그 생성
"<p>직접생성</p>"
생성된 객체 추가/이동
...
$(생성객체).appendTo('선택자')	생성객체를 선택자의 첫번째 자식으로 추가
...
CSS : HTML로 작성된 문서에 대한 스타일(글자 크기, 색상, 여백 등)을 적용하는 언어
JavaScript : HTML로 작성된 웹페이지 문서에 기능을 적용하는 언어
jQuery : JavaScript의 문법을 간소화하기 위한 프레임워크
ECMAScript : ECMA 표준화 단체에서 JavaScript를 기초로 하여 구성한 스크립트 표준
HTML
화면구현 시 고려해야할 사항
웹 표준
...
시멘틱 태그
header : 문서의 header를 나타낼 때 사용. 사이트 로고, 링크, 소개 등을 넣음.
nav : 문서의 navigation을 나타낼 때 사용. 메뉴영역으로 사용.
section : 문서의 컨텐츠가 들어가는 영역. 컨텐츠를 주제별로 묶을때 사용.
aside : 본문 전체 내용과 직접적인 연관성이 없는 분리된 내용을 담을 때 사용. 배너 광고, 위젯.
footer : 문서의 하단을 나타낼 때 사용. 주소, 연락처, 저작권 등이 입력됨.
CSS
선택자
아이디 선택자 : #
클래스 선택자 : .
...
종류	설명
toncat	Apache Software Foundation에서 Servlet과 JSP를 통한 동적인 웹문서를 처리하기 위해 만든 웹 어플리케이션 서버
...
종류	설명
jsp
(Java Server Pages)	SUN사의 JAVA를 기반으로 웹 환경에 맞게 개발된 동적 웹 프로그래밍 언어
php
(Projessional
Hypertext
Preprocessor)	Rasmus Lerdorf가 개발한 동적 웹페이지 생성 스크립트 언어
C, C++, Perl, Java등의 언어를 기반으로 하여 만들어 졌고, C언어 문법과 유사
asp
(Active Server Pages)	Micro Soft에서 개발한 동적 웹 페이지 생성 스크립트 언어
Web Server VS WAS
구분	장점	단점
Web Server	- 요청에 대한 결과 페이지만 전송하기 떄문에 처리속도가 빠름
- HTML같은 단순 문서로만 구성되어 있어 구현이 쉬움	- 만들어진 정보만 보여주기 때문에 서비스가 한정적
- 문서의 내용이 변경될 경우 직접 수정
WAS	- 여러 데이터 활용이 가능하여 다양한 서비스 제공 가능
- 문서의 내용이 변경될 경우 직접 수정하지 않음	- 데이터를 처리하여 결과를 전송해야 하기 때문에 처리속도가 느림
- 서비스에 해당하는 소스를 직접 작성해야 하기 때문에 구현이 어려움
개발 환경 구축
현재 자바 버전 확인
Window -> Preferences -> Java -> Installed JREs -> jre 1.8 확인
아파치-톰캣 서버 버전 별 지원 정보 확인
...
HTTP 프로토콜을 사용하는 서블릿은 javax.servlet.http.HttpServlet클래스를 상속받는데 HttpServlet클래스는 javax.servletGenericServlet을 상속하여 구현된 클래스다.
...
서블릿 라이프 사이클
첫번째 요청인 경우, 객체를 생성하며 init() 메소드 호출
이후 작업이 실행 될 때마다 HTTP Type에 따른 doGet(), doPost() 메소드 호출
최종적으로 서블릿이 서비스 되지 않을 때(서버가 꺼질 때) destroy() 메소드 호출
※ destroy() 메소드는 보통 서버가 종료되었을 때, 내용이 변경되어 재컴파일 될 때 호출
20200423120125
Servlet
동작 순서
전송 값에 한글이 있을 경우를 대비하여 인코딩 처리
request.setCharacterEncoding("utf-8");
웹에서 보내준 데이터를 꺼내서 변수에 저장
String test = request.getParameter("test");
String test3 = request.getParameter("test3");
System.out.println(test + "/" + test3);
비지니스 로직 처리(Service 호출)
처리 결과 내보내기(Response)
...
예시	out.println(“”);	<% for(int i=0;i<10;i++) { %>
JSP 실행 방식
...
resopnse	HttpServletResponse 객체 참조 변수
...
JSP 내장객체의 영역(scope)
내장 객체	설명
page	하나의 JSP페이지를 처리할 때 사용되는 영역(한 페이지)
request	하나의 요청을 처리할 때 사용되는 영역(다음 페이지)
...
표준 액션 태그	커스텀 액션 태그
사용법	- JSP 페이지에서 바로 사용
- 태그 앞에 jsp 접두어가 붙음	- 별도의 라이브러리 설치 필요
- 라이브러리 선언에 맞는 접두어가 붙음
예시	jsp:include page="header.jsp"	c:set var="cnt" value="0"
- 접두어란 태그 이름 앞에 XXX: 형식으로 제공하는 태그의 그룹을 지정하는 것 - 예시를 사용할 때는 양 옆에 "<"와 "/>"를 넣을것
표준 액션 태그
JSP에서 기본으로 제공하는 태그
태그 이름	설명
jsp:include	현재 페이지에 특정 페이지를 포함 할 때 사용
jsp:forward	현재 페이지 접근 시 특정 페이지로 이동 시킬 때 사용
jsp:param	jsp:incpude, jsp:forward의 하위 요소로 사용되며, 해당 페이지에 전달할 값을 기록할 때 사용
...
Formatting
Tags	- 메시지 형식이나 숫자, 날짜 형식과 관련된 포맷 방식을 제공
- <%@ taglib url="http://java.sun.com/jsp./jstl/fmt" prefix=“fmt” %>
Function Tags	- trim, substring과 같은 문자열 처리 함수를 제공
- <%@ taglib url="http://java.sun.com/jsp./jstl/function" prefix=“fn” %>
XML Tags	- 데이터의 XML 파싱 처리 등 XML 문서를 다루는 함수를 제공
- <%@ taglib url="http://java.sun.com/jsp./jstl/xml" prefix="x" %>
SQL Tags	- 페이지 내에서 DB를 연동하고, 쿼리를 실행할 수 있는 함수 제공
...
속성	설명
current	현재 반복 횟수
index	반반복 라운드의 제로기반 인덱스(0 ~ n-1)
count	반복 라운드의 1기반 인덱스(1 ~ n)
...
페이지 표현 방식(15 : 1 2 3 4 5, 610 : 6 7 8 9 10) : reqPage에 종속됨
DB에서 조회해올 값
해당 페이지의 게시물 List
전체 게시물 수 : totalPage
SELECT *
...
request를 MultipartRequest객체로 변경
...
- 매개변수 종류 
  ...
  4. 인코딩
  5. 중복 파일 발견시 처리 방법
오리지널 파일 이름과 실제 저장되는 파일의 이름을 각각 저장
...
게시글 수정 중 파일 처리
파일 상태를 위한 if 및 input[type=hidden] 태그 추가
...
Servlet에서 새 파일이 들어오지 않은 상태에서의 기존 값 유지
...
ajax의 장단점
장점
비동기식 방식으로 웹서버의 응답을 기다리지 않고 데이터를 빠르게 처리
페이지 리로딩 없이 처리
단점
한 페이지에 지속적으로 사용 시 리소스가 쌓여 페이지가 느려짐
...
속성명	내용
200(OK)	요청 성공
404(Not Found)(loading)	페이지 없음
500(Internal Server Error)(loaded)	서버 오류 발생
처리절차
srcipt문에 요청을 위한 XMLHttpRequest 객체 생성
...
jQuery ajax
$.ajax()를 이하여 처리
url 속성을 통해 전송할 url 주소 설정
data 속성을 통해 전달할 데이터 설정
성공, 실패 시 시 처리할 로직을 함수로 선언
반드시 처리할 로직을 선언
$.ajax()의 주요 속성
속성명	내용
...
백엔드 문서용으로 사용하기 좋다. -> 진행하던 프로젝트를 다른 팀이 맡을 경우 유용
UML의 종류
종류	설명
클래스 다이어그램	시스템을 구성하는 클래스
...
Aggregation	─────────◇	클래스간의 지역변수에서 참조하고 는 관계로 두 클래스의
라이프사이클이 다를 때
Composition	─────────◆	클래스간의 지역변수에서 참조하고 는 관계로 두 클래스의
라이프사이클이 같을 때
...
Relation	액터와 유스케이스 사이의 의미 있는 관계
Actor
...
포함관계(include)는 하나의 유스케이스가 다른 유스케이스의 실행을 전제로 할 때 형성되는 관계로, 해당 유스케이스를 실행하기위해 반드시 실행되어야 하는 경우 적용 20200508173426
Relation - 의존관계2(extend)
확장관계(extend)는 확장 대상 유스케이스를 수행할 때 특정 조건에 따라 확장 기능 유스케이스를 수행하는 경우 적용 20200508173526
Relation - Generalization
일반화 관계(Generalization)는 유사한 유스케이스 또는 액터를 모아 추상화한 유스케이스 또는 액터와 연결시켜 그룹을 만들어 이해도를 높이기 위함 20200508173726
유스케이스 다이어그램 작성 순서
액터 식별
...
유스케이스 정의서 포함항목
유스케이스명 : 액터가 시스템을 통해 달성할 목적을 명확하게 표현
액터명 : 시스템에서 수행하는 역할 이름
개요 : 유스케이스를 수행하는 개요
사전조건 : 올바르게 동작하기 위해 사전에 충족되어야 하는 조건
사후조건 : 유스케이스가 실행된 후 만족해야 하는 조건 기술
기본흐름 : 시스템과 엑터 사이에 목적을 달성하기 위한 기본적인 상호흐름을 기술하며, 오류나 예외가 발생하지 않는 것을 전제로 작성
대체흐름 : 기본흐름으로 부터 경우에 따라 선택적으로 실행되고, 다시 기본흐름으로 돌아오거나, 예외가 발생한 경우 이를 처리하는 흐름 기술
...
- 유지보수가 쉬움	- 너무 의존하는 경우 개발자들의 능력이 떨어져 스스로 직접 개발하는 것이 어려워 짐
- 습득에 걸리는 시간이 필요함
Framework 종류
구분	설명	종류
영속성
프레임워크	데이터의 저장, 조회, 변경, 삭제를 다르는 클래스 및 설정 파일들을
라이브러리화 하여 구현한 프레임워크	- Mybatis
- Hibernate
자바
프레임워크	JAVA EE를 통한 웹 어플리케이션 개발에 초점을 맞추어 필요한
요소들을 모듈화 하여 제공하는 프렘임워크	- spring
- 전자정부표준
- Struts
화면 구현
프레임워크	Front-End를 보다 쉽게 구현할 수 있게 틀을 제공하는 프레임워크	- Bootstrap
- Foundation
- MDL
기능 및 지원
프레임워크	특정 기능이나 업무 수행에 도움을 줄 수 있는 기능을 제공하는
프레임워크	- Log4j
- JUnit5
Mybatis
정의
...
- Mybatis를 이용하기 위한 설정들이 저장되어 있는 파일
- xml 파일 상단에 mybatis-config 파일이라는 선언


<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE configuration PUBLIC "-//mybatis.org//DTD Config 3.0//EN" "http://mybatis.org/dtd/mybatis-3-config.dtd">	- 실제 Database에서 수행할 query문들을 저장하는 파일
- xml 파일 상단에 mybatis-mapper 파일이라는 선언
...
태그명	설명
<configuration>	최상위 태그로 내부에 필요한 설정들을 작성
...
<typeAliases>	Mybatis에서 사용할 자료형의 별칭을 선언
Example	<typeAliases>
    <!-- type에는 패키지 명까지 전부 기술해주어야 한다. -->
    <typeAlias type="member.model.vo.Member" alias="Member" />
</typeAliases>
<mappers>	사용하고자 하는 쿼리가 정의된 mapper 파일을 등록
Example	<mappers>
    <mapper resource="member/model/mapper/membermapper.xml" />
...
Example	<environments default="development">
...
<transactionManager>	트랜잭션 관리를 누가 할 것인가에 대한 설정
JDBC : JDBC commit과 rollback을 처리하기 위해 connection을 가지고 온다(직접 처리)
MANAGED : commit과 rollback을 하지 않고, 대신 컨테이너가 트랜잭션을 관리하고 커넥션을 닫아주는 설정
<dataSource>	데이터 베이스 연결을 관리하는 설정
POOLED : Connection 객체를 pool영역에 저장해 두고 이후 Connection 객체를 생성할 때 이를 재사용 한다.
UNPOOLED : Connection 객체를 별도로 저장하지 않고, 객체 호출 시 매번 새로 생성하여 사용
...
Spring 특징
DI(Dependancy Injection/의존성 주입) : 설정 파일이나 어노테이션을 통해 각 객체간 의존 관계를 설정하여 개발자가 직접 의존하는 객체를 생성할 필요가 없음
Spring AOP(관점지향 프로그래밍) : DataBase의 트랜잭션 처리나 로깅처리와 같이 여러 모듈에서 공통으로 필요로 하는 기능의 경우 해당 기능을 분리하여 관리함
Spring JDBC : Mybatis나 Hibernate 등의 데이터베이스를 처리하는 영속성 프레임워크와 연결할 수 있는 인터페이스를 제공
Spring MVC : MVC 디자인 패턴을 통해 웹 어플리케이션의 Model, View, Controller 사이의 의존 관계를 DI 컨테이너에서 관리하여 개발자가 아닌 서버가 객체들을 관리하는 어플리케이션을 구축할 수 있음
Spring IoC, DI
IoC
...
BeanFactory	- 자바 빈 객체의 등록 및 관리
- getBean() 메소드를 통해 객체를 가져옴
ApplicationContext	- BeanFactory의 확장 개념
- Spring의 각종 부가 서비스를 제공
- 일반적인 IoC컨테이너를 의미
GenericXml
ApplicationContext	- ApplicationContext를 구현한 클래스
- 일반적인 XML형태의 문서를 읽어 컨테이너 역할을 수행
Spring DL
...
- 개발자 간 XML 설정 파일의 충돌을 최소화	- 어플리케이션에 등록될 bean이 어떤 것들이 있고, bean들 간의 의존관계가 어떻게 되는지 한눈에 파악할 수 없음
bean 등록 Annotation
Annotation	설명
@Component	-객체를 나타내는 일반적인 타입으로 bean태그 역할
@Controller	- Presentation Layer Annotation으로 view에서 전달된 웹 요청과 응답을 처리하는 클래스에 사용(Servlet)
@Service	- Service Layer Annotation으로 비지니스 로직을 가진 클래스에 사용(Service)
@Repository	- Persistence Layer Annotation으로 영속성(파일, DB)을 가진 클래스에 사용(DAO)
- @Controller, @Service, @Repository는 특정한 객체의 역할에 대한 @Component의 구체화된 형태
bean 의존 관계 주입 Annotation
Annotation	설명
@Autowirde	- 주로 변수 위에 설정하여 해당 타입 객체를 컨테이너에서 찾아서 자동으로 주입
@Inject	- @Autowired와 동일한 기능 지원
- @Autowired Spring 전용, @Inject는 Java전용
@Qualifier	- @Autowired와 같이 사용되며, 특정 객체의 이름을 이용하여 의존성을 주입할 때 사용
- Autowired를 하려는 상황에 해당 타입의 객체가 컨테이너에 2개 이상 존재하는 경우 어떤 객체를 주입할지 결정할 수 없어 에러가 발생하는데 이를 해결함
...
Spring MVC 요소	설명
DispatcherServlet	- 유일한 Servlet 클래스로서 HTTP 프로토콜을 통해 들어오는 모든 요청을 가장
먼저 처리하는 Front Controller
HandlerMapping	- 클라이언트 요청을 어떤 Controller가 처리할 지 URL Mapping값을 가지고 있음
Controller	- 실질적인 클라이언트 요청을 처리
ViewResolver	- Controller가 반환한 View 이름으로 실행 될 jsp 경로를 가지고 있음
View	- 실제 클라이언트들에게 보여질 페이지
20200528165442

Spring MVC 프로젝트 폴더 구조 20200528165523
...
apache-maven-3.5.3 폴더 안에 repository 폴더 생성
conf - settings.xml파일 열기
<local 검색 -> localRepository 태그 복사 주석 밖에 붙여넣기, 태그 내용 삭제
2에서 만든 repository 폴더 주소 태그 내용에 복사 붙여넣기
sts-bundle 폴더 아래 sts-3.9.12.RELEASE 폴더 아래 STS.ini 파일 열기
org.springsource.sts.ide 아래 -vm C드라이브 Java 폴더 jre의 bin 폴더 경로 복사 붙여넣기\javaw.exe
STS.exe 실행 후 기존 이클립스와 동일하게 workspace 생성
Window - Preferences - Maven - User Settings
User Settings에 3에서 설정한 settings.xml 설정
Window - Preferences - Team - Ignored Resources
Add Pattern - "/target/" 추가
Servers 옆 boot dashboard 제거
Encoding UTF-8 설정하기
Server - Runtime Environment 있던 서버 삭제하고 기존 Apache 서버 등록, 서버 생성
Spring Project는 Spring Legacy Project로 생성
Project Name 설정 후 Spring MVC Project 선택 후 NEXT
도메인 주소 등록. 도메인은 뒤에서부터 입력(naver.com -> com.naver)
prom.xml - overview 선택
java-version을 1.8로 변경
org.springframework-version을 5.0.6.RELEASE로 변경
프로젝트 우클릭 - Preference - Project Facets
Java - 1.8 변경
우측 Runtime에서 아파치 서버 선택
프로젝트 - src - main - webapp 밑에 jsp 파일 생성(기존 webcontent)
서버 더블 클릭 - Modules - Path "/"로 변경
라이브러리 추가
...
용어	설명
Joinoint	- 클라이언트가 호출하는 모든 비지니스 메소드
- 일반적으로 Srvice의 모든 클래스
Pointcut	- 필터링 된 조인 포인트
- Joinpoint 중 공통기능을 적용할 선택된 메소드
Advice	- Pointcut에 적용할 공통 기능의 코드
Aspect or Advisor	- Pointcut + Advice = Aspect
- 어떤 Pointcut에 어떤 Advice를 적용할지 결정
- Advisor는 Aspect와 같지만 몇몇 특수한 경우에 사용(트랜잭션 처리)
...
*Service	- 클래스명(Service로 끝나는 모든 클래스)
*	- 메소드 명
(..)	- 매개변수
형식	예	설명
리턴 타입	*	- 모든 리턴 타입 허용
...
MemberService+	- 클래스 이름 뒤에‘+’가 붙으면 해당 클래스로 부터 파생된 모든 자식클래스를 선택
- 인터페이스 뒤에 ‘+’가 붙으면 해당 인터페이스를 implement한 모든 클래스 선택
...
After Returning	- 비지니스 메소드가 성공적으로 리턴 되면 동작
...
메소드	설명
proceed()	- proceed() 메소드는 비즈니스 메소드를 수행하는 메소드로 Object 타입 객체를 리턴 하는데 이 Object 객체가 비즈니스 메소드 수행 후 리턴 하는 객체
- 다른 advice는 proceed()메소드가 필요 없지만, Around의 경우 비즈니스 로직 수행 전/후 로직을 모두 처리하기 때문에 비즈니스 메소드를 수행하는 proceed() 메소드가 필요하여 반드시 ProceedingJoinPoint 인터페이스가 필요
AOP 적용하기
...
AOP 설정 적용
AroundLog 클래스를 testAop라는 id로 bean 생성(AOP로 적용할 기능을 작성한 클래스)
<aop:config> : AOP 설정정보임을 나타냄
<aop:aspect> : aspect를 설정 → testAop를 이용하여 기능을 적용
<aop:around> : around 적용
...
<aop:around>	- 메소드가 정상적으로 실행되는지 또는 예외를 발생시키는지 여부에 관계없이 실행되는 어드바이스 정의
<aop:after-returning>	- 메소드가 정상 실행된 후 적용되는 어드바이스 정의
<aop-throwing>	- 메소드가 예외를 발생시킬 때 적용되는 어드바이스 정의
Annotation을 이용한 AOP 설정
클래스 선언부에 @Aspect 어노테이션 정의
해당 클래스를 객체생성해야 사용이 가능하므로 @Component 어노테이션도 함께 정의
xml파일에 annotation 설정 입력(<aop:aspectj-autoproxy/>)
Spring AOP - Advice 정의하는 어노테이션
용어	설명
@Before("pointcut")	- 타겟 객체의 메소드가 실행되기 전에 실행되는 어드바이스
- JoinPoint를 통해 파라미터 정보 참조 가능
@After("pointcut")	- 타겟 객체 메소드가 실행되고나면 성공여부와 관계없이 모두 호출되는 어드바이스로, 반환값을 받을 수 없다.
@Around("pointcut")	- 타겍 객체의 메소드 호출 전과 후에 실행될 코드를 구현할 어드바이스
- ProceedingJoinPoint를 통해 파라미터와, 반환값 모두 참조 가능
@AfterReturning
("pointcut", returning="")	- 타겟 객체의 메소드가 정상 실행을 마친 후 호출되는 어드바이스
- 리턴값을 참조할 때는 returning 속성에 리턴값을 저장할 변수명을 지정
@AfterThrowing
("pointcut", throwing="")	- 타겟 객체의 메소드가 예외가 발생하면 호출되는 어드바이스
...
application.xml namespace 추가 : websocket
웹소켓 설정 추가
<!-- 웹소켓 설정 -->
<websocket:handlers>
...
임베디드 시스템 특징
운영체제 경량성 : 고속, 경량 동작과 단순 운영을 목표로 하기 때문에, 일반적인 시스템과 달리 최소한의 Computing Power만을 사용하며, 단순한 OS가 많이 활용
개별환경 독립성 : 임베디드 소프트웨어는 경량 운영체제에서 동작하기 때문에 컴파일러 등의 building 환경이 모두 제거되어 있다.
아두이노
정의
...
아두이노 보드 종류
아두이노 UNO 보드 : 표준 보드
아두이노 Nano : UNO의 소형화 버전
아두이노 Mega2560 : UNO의 확장버전
아두이노 pro/pro mini : UNO의 초소형화 버전
아두이노 쉴드
...
아두이노를 통한 IoT
Ethernet : LAN(Loacl Area Network)을 위해 개발된 근거리 유선 네트워크 통신망 기술로 CSMA/CD 프로토콜을 이용하여 통신하며 IEEE 802.3에 표준으로 정의되어 있음
WiFi
아두이노 블루투스
...
Web/WAS
WebServer : 웹 브라우저의 요청을 받아서 html파일, 이미지, 자바스크립트 등의 정적인 컨텐츠를 제공
WAS(Web Application Server) : 서버에서 어플리케이션이 동작할 수 있는 환경을 제공, 동적인 컨텐츠 제공이 가능(웹 서버와의 차이점).
...
결합도/응집도
결합도 : 모듈과 모듈 간의 어느정도 관련성이 있는지 나태내며, 관련이 적을수록 모듈의 독립성이 높아 영향이 적어짐
응집도 : 모듈 내부에서 구성요소간에 밀접한 관계를 맺고 있는 정도, 응집도는 높을수록 필요한 요소들로 구성되어 있다
코드 : 특정 데이터를 사용 목적에 따라 그룹으로 분류 및 나열하고 특정 자료의 선별 및 추출작업을 용이하게 하기 위해 부여한 숫자, 문자, 기호
코드의 기능
식별 : 각 데이터간의 성격에 따라 구분
...
maven에서 설치되는 library가 저장된 폴더 이동 : org/projectlombok/lombok/1.16.20
lombok-1.16.20.jar 실행
Install/Update 선택
getter, setter, 생성자를 만들 VO에 @Data Annotation 추가
...
단위 테스트 : 테스트가 가능한 단위로 작게 분리된 소프트웨어 내에서 결함을 찾고 검증하는 테스트 활동
통합 테스트 : 컴포넌트 간 인터페이스를 테스트하고 운영체제, 파일 시스템, 하드웨어 또는 시스템 간 인터페이스와 같은 각각 다른부분과 상호 연동이 정상적으로 작동하는지 여부를 테스트하는 것
시스템 테스트 : 통합된 단위 시스템의 기능이 시스템에서 정상적으로 수행되는지를 테스트하는 것으로 성능 및 장애 테스트가 여기에 포함
기능적 요구사항 / 비기능적 요구사항
기능적 요구사항 : 요구사항 명세서, 비지니스 절차, 유스케이스 등 명세서 기반의 블랙박스 테스트
비기능적 요구사항 : 성능 테스트, 회복 테스트, 보안 테스트, 내부 시스템의 메뉴 구조, 네비게이션 구조 등 구조적 요소에 대한 화이트박스 테스트 수행
블랙박스 테스트 / 화이트박스 테스트
블랙박스 테스트 : 애플리케이션 내부 구조나 작동원리는 모르는 상태에서 동작을 검사(사용자 관점)
화이트박스 테스트 : 애플리케이션 내부 구조나 작동원리를 검사하는 테스트(개발자 관점)
인수 테스트 : 일반적으로 최종 사용자와 업무에 따른 이해 관계자 등이 테스트를 수행함으로써 개발된 제품에 대해 운영여부를 결정하는 테스트로, 실제 업무 적용 전에 수행
...
테스트 기반에 따른 테스트의 종류
구조 기반 테스트 : 소프트웨어 내부 논리의 흐름에 따른 테스트 케이스 작성 및 결함 발견활동
명세 기반 테스트 : 사용자 요구사항 분석시에 주어진 명세를 빠뜨리지 않고 테스트 케이스화
경험 기반 테스트 : 유사 소프트웨어나 기술서의 테스터의 경험, 직관, 기술 능력을 바탕으로 하는 테스트 기법
테스트 자동화
...
테스트 도구의 장점
테스트 데이터의 재입력과 재구성 같은 반복작업의 자동화를 통해서 테스트 인력과 시간을 최소화
향상된 요구사항 정의 성능 및 스트레스 테스트, 품질 측정의 최적화
빌드확인, 회귀, 다중 플랫봄 호환성, 소프트웨어 구성, 기본 테스트 등 향상된 테스트 품질 보정
테스트 종료 기준
작성된 모든 단위 시험 케이스가 성공으로 기록되거나, 몇몇 단위 시험 케이스가 통과되지 않았지만 그 영향이 제한적으로 판단되는 경우
중대한 결함이 만족스럽게 조치되었고 테스트 팀의 재테스트 결과, 발생한 결함이 조치되었다고 판단되는 경우
수정되지 않은 결함이 존재하더라도, 전체 결함의 검토 결과 그러한 결함이 제한적인 것으로 확인되는 경우
Spring 테스트 방법
junit 라이브러리 버전 4.12로 변경
...
6. 테이블 설계
학생들 관리할 테이블
학생번호 : STU_NO/NUMBER/PRIMARY KEY
이름 : NAME/VARCHAR2(20)/NN
나이 : AGE/NUMBER/NN CHECK 0 초과
성별 : GENDER/CHAR(3)/CHECK '남', '여'
전화번호 : PHONE/CHAR(13)/DEFAULT '010-0000-0000'
주소 : ADDR/VARCHAR2(500)/NN
게시판
게시물 번호 : BOARD_NO/NUMBER/PRIMARY KEY
게시물 제목 : BOARD_TITLE/VARCHAR2(300)/NN
게시물 내용 : BOARD_CONTENT/VARCHAR2(3000)/NN
게시물 작성자 : BOARD_WRITER/NUMBER/FORIGN KEY
작성일 : WIRTE_DATE/DATE/DEFAULT SYSDATE
7. DB 연결 단계
드라이버 등록
Connection 객체 생성
Statemnt 객체 생성 -> PreparedStatement 객체 생성 후 위치홀더에 값을 대입
쿼리문 요청 후 결과 받기 -> 쿼리문 실행 및
결과처리
자원 반환
8. 사이트
테이블 정의 사이트
